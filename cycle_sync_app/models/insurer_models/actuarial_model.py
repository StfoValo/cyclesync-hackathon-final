...
class ActuarialModel:
    ...
    def get_esg_sustainability_metrics(self, portfolio_data=None, bev_data=None):
        """
        ...
        # --- FIX: Stop using random numbers. Use the REAL global sums! ---
        if not portfolio_data:
            portfolio_data = self.get_asset_risk_portfolio()
        if not bev_data:
            bev_data = self.fleet_model.get_bev_regional_analytics(0)

        tires_replaced = portfolio_data['global']['tires']['Critical (<2mm)']
        brakes_replaced = portfolio_data['global']['brakes']['Critical (<3mm)']
        batteries_replaced = int(sum(region['cohorts']['0-3_months'] for region in bev_data))
        
        # 2. CIRCULAR ECONOMY LEDGER
        payload = {
            "emissions": {
                "baseline_co2_tons": total_baseline_tons,
                "real_telematics_co2_tons": real_co2_tons,
                "co2_saved_tons": co2_saved_tons,
                "equivalent_trees_planted": int(co2_saved_tons * 45)
            },
            "circular_economy": {
                "recovered_tires": {
